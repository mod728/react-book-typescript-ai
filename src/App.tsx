...
const App = () => {
    ...
    const getWeather = (e: React.SubmitEvent<HTMLFormElement>) => {
        e.preventDefault()  
        setLoading(true)
        fetch(`https://api.weatherapi.com/v1/current.json?key=3xxxyyyzzz61305&q=${city}&aqi=no`) 
            .then(res => res.json())
            .then(data => {
                setResults({
    ...
    const getAiSuggestion = (city: string, temperature: string) => {
        ...
        fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            headers: {
                ...
                "Authorization": "Bearer sk-or-v1-ea791a6b6w09fwowe9wewlwlm"
            },
            body: JSON.stringify({
    ...
    return (
        <div className="wrapper">  
            <div className="container">
                ...
                {loading ? <Loading loadingStyle="weather-loading"/> : <Results results={results}/>}
                {aiLoading ? <Loading loadingStyle="ai-loading"/> : <AiSuggestion aiSuggestion={aiSuggestion}/>}
            </div>
        </div>
    )
}
